...
def fill_space(sent, n):
    diff = n - len(sent)
    filled = sent.replace(' ', '  ', diff)
    return filled if len(filled) == n else fill_space(filled, n)
...
text_wrap = textwrap.wrap(text, width=n)
text_output = [fill_space(sent, n) for sent in text_wrap]

pprint.pprint([(w, len(w)) for w in text_output])
